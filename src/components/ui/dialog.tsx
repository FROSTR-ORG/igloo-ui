...
export function Dialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      >
        {title ? (
          <div id={titleId} className="mb-2 text-lg font-semibold text-igloo-text">
            {title}
          </div>
        ) : null}
        {description ? (
          <p id={descriptionId} className="mb-4 text-sm text-igloo-muted">
            {description}
          </p>
        ) : null}
        {children}
    ...
    <Dialog open={open} onClose={onCancel} className="max-w-md" ariaLabelledBy={titleId}>
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <h3 id={titleId} className="text-lg font-semibold text-igloo-text">{title}</h3>
          <p className="mt-2 text-sm text-igloo-muted">{message}</p>
        </div>
      </div>
